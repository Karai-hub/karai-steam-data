#!/usr/bin/env python3
"""
Steam giveaway hunter v0.2 for Karai.

Changes vs v0.1:
- Expired offers are rejected by our own clock, regardless of GamerPower status.
- Steam-key giveaways are classified separately from direct Steam-store giveaways.
- Cosmetic / currency / promo-only DLC is filtered more aggressively.
- DLC lookup falls back to the base-game title when direct title matching fails.
- External-key giveaways are never treated as "Steam price must be zero".
"""
...
USER_AGENT = "KaraiSteamHunter/0.2"
...
def clean_title(title):
    ...
    title = re.sub(r"\s*Steam\s+Key\s*$", "", title, flags=re.I)
    title = re.sub(r"\s*\(Steam\)\s*Key\s*$", "", title, flags=re.I)
...
def is_obvious_reject(item):
    ...
    if content_kind == "cosmetic_or_promo_dlc":
...
def base_game_title_from_dlc(title):
    """
    Conservative fallback: use the part before ' - ' or ':' for DLC names.
    Example: 'DAVE THE DIVER - Godzilla Content Pack' -> 'DAVE THE DIVER'
    """
    if " - " in title:
...
def recommendation_band(candidate, score):
    ...
    # External key: Steam price does NOT need to be zero.
    if delivery == "external_steam_key":
        if verification in {"reject_non_full_product"}:
            return "skip"
        if score >= 5:
            return "likely_match"
        if score >= 2:
