#!/usr/bin/env python3
"""
Steam giveaway hunter v0.4 for Karai.

Focus:
- strip leftover "(Steam)" suffixes from titles;
- classify service/account rewards separately from real DLC;
- reject promo/service DLC such as starter kits, temporary buffs, premium currency,
  emblems/codes and similar account rewards;
- keep gameplay DLC, full games and meaningful permanent DLC for later scoring.
"""
...
USER_AGENT = "KaraiSteamHunter/0.4"
...
def clean_title(title):
    ...
    # Remove trailing giveaway wording.
    t = re.sub(r"\s*Giveaway\s*$", "", t, flags=re.I)

    # Remove Steam/PC wrappers whether they appear before "Giveaway" or remain
    # after earlier normalization.
    ...
    # One more pass in case removing "Key" exposed "(Steam)".
    t = re.sub(r"\s*\((?:Steam|PC)\)\s*$", "", t, flags=re.I)
...
def main():
    ...
    for item in matches[:20]:
        ...
        print(
            f"- [{band}] [{item.get('content_kind')}] "
            f"[{item.get('delivery')}] [{verification}] {item.get('title')}"
        )
