...
USER_AGENT = "KaraiSteamHunter/1.0"
...
LOOT_REJECT_PHRASES = (
    "skin", "skins", "avatar", "wallpaper", "soundtrack", "artbook",
    "art book", "booster", "currency", "coins", "gems", "cosmetic"
)
...
POSITIVE_TAG_WORDS = {
    ...
    "colony": 4, "stealth": 3, "simulation": 1,
    "simulator": 1, "crafting": 1, "collect": 1,
    "adventure": 1, "visual novel": 2
}
NEGATIVE_TAG_WORDS = {
...
def clean_title(title):
    title = re.sub(r"\s*\((?:Steam|PC)\)\s*Giveaway\s*$", "", title, flags=re.I)
    title = re.sub(r"\s*Giveaway\s*$", "", title, flags=re.I)
    return re.sub(r"\s+", " ", title).strip()

def text_blob(item):
    return " ".join(str(item.get(k, "")) for k in
                    ("title", "description", "instructions", "type", "platforms")).lower()
...
def is_obvious_reject(item):
    ...
    if str(item.get("type", "")).lower() == "loot":
        for phrase in LOOT_REJECT_PHRASES:
            if phrase in blob:
                return True, f"loot_reject:{phrase}"
    return False, None
...
def title_similarity(a, b):
    ta = set(re.findall(r"[a-zа-я0-9]+", a.casefold()))
    tb = set(re.findall(r"[a-zа-я0-9]+", b.casefold()))
    if not ta or not tb:
        return 0.0
...
def best_steam_match(title):
    ...
    for item in items[:10]:
        ranked.append((title_similarity(title, str(item.get("name", ""))), item))
    if not ranked:
        return None
    ...
    if score < 0.45:
        return None
    return {"appid": item.get("id"), "name": item.get("name"), "search_match_score": round(score, 3)}

def inspect_steam_ru(appid):
    payload = http_json(STEAM_APPDETAILS_URL, {"appids": appid, "cc": "RU", "l": "russian"})
    wrapper = payload.get(str(appid)) if isinstance(payload, dict) else None
    if not wrapper or not wrapper.get("success"):
        return {"checked": True, "reachable": False, "verification": "unknown",
    ...
    initial, final, discount = price.get("initial"), price.get("final"), price.get("discount_percent")

    if app_type in {"game", "dlc"} and isinstance(initial, int) and isinstance(final, int)        and initial > 0 and final == 0 and discount == 100:
    ...
    return {
        "checked": True, "reachable": True, "verification": verification,
        "reason": reason, "steam_type": app_type, "steam_name": data.get("name"),
        "is_free": is_free, "price_overview": price,
...
def main():
    ...
    for source_item in raw:
        ...
        try:
            ...
            match = best_steam_match(candidate["title"])
        except Exception as exc:
            match = None
            candidate["steam_lookup_error"] = type(exc).__name__

        candidate["steam_match"] = match

        if not match or not match.get("appid"):
            candidate["steam_ru"] = {"checked": False, "verification": "needs_review",
                                     "reason": "no_confident_steam_appid_match"}
        else:
            appid = int(match["appid"])
            if appid in owned_game_appids or appid in owned_dlc_appids:
                candidate["ownership"] = {"owned": True, "reason": "appid_match", "appid": appid}
                continue
            try:
                time.sleep(REQUEST_DELAY_SECONDS)
                candidate["steam_ru"] = inspect_steam_ru(appid)
            except Exception as exc:
                candidate["steam_ru"] = {"checked": True, "reachable": False,
        ...
        candidate["taste"] = {
            "score": score, "positive_signals": positives,
            "negative_signals": negatives, "band": band
        }
    ...
    band_order = {
        "must_claim": 0, "likely_match": 1, "conditional": 2,
        "family_only": 3, "needs_review": 4, "low_priority": 5, "skip": 6
    }
    ...
    save_json(MATCHES_FILE, {
        ...
        "bands": {
            band: sum(1 for item in matches if (item.get("taste") or {}).get("band") == band)
    ...
    print(f"Filtered Steam candidates: {len(matches)}")
    for item in matches[:20]:
        band = (item.get("taste") or {}).get("band")
        verification = (item.get("steam_ru") or {}).get("verification")
        print(f"- [{band}] [{verification}] {item.get('title')}")
